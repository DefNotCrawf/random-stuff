...
alphabet = string.ascii_letters + string.digits + string.punctuation # all possible characters

while True: # generate password
	...
	if (sum(c.islower() for c in password) >=4
			...
			and sum(c in string.punctuation for c in password) >=(c_length//3)): # password requirements
		break
print(password)

if input("Do you want to copy the password to clipboard? (y/n): ").lower() == "y": # copy to clipboard
